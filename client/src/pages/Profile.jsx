...
const Profile = () => {
  ...
  const [favoritosCampeones, setFavoritosCampeones] = useState([]);
  const [itemsDatos, setItemsDatos] = useState({});
  const [runesDatos, setRunesDatos] = useState({});
  const [favoritosItems, setFavoritosItems] = useState([]);
  const [favoritosRunas, setFavoritosRunas] = useState([]);
  const [misiones, setMisiones] = useState([]);
  const [resumenMisiones, setResumenMisiones] = useState(null);
  const [actualizandoDecoracion, setActualizandoDecoracion] = useState(null);

  const [modalTipo, setModalTipo] = useState('items');
  ...
  useEffect(() => {
    ...
    const cargarPerfil = async () => {
      try {
        ...
        setFavoritosCampeones(favoritosResp);
  ...
  const manejarCargaArchivo = (tipo) => (evento) => {
  ...
  const abrirModalFavoritos = (tipo) => {
    setModalTipo(tipo);
  ...
  const alternarSeleccionTemporal = (id) => {
  ...
  const renderModalElemento = (id, nombre, icono) => {
    ...
    return (
      <Box
        ...
        p={3}
        cursor="pointer"
        borderColor={seleccionado ? 'gold.400' : 'background.muted'}
        bg={seleccionado ? 'yellow.900' : 'transparent'}
        _hover={{
          borderColor: seleccionado ? 'gold.400' : 'gold.300',
          bg: seleccionado ? 'yellow.900' : 'background.secondary',
        }}
      ...
      >
        <Flex align="center" gap={3}>
          {icono}
          <Text fontWeight="medium" color={seleccionado ? 'gold.200' : 'foreground.primary'}>
            {nombre}
          </Text>
        </Flex>
      </Box>
      ...
      <SimpleGrid columns={{ base: 2, md: 3, lg: 4 }} spacing={4}>
        {favoritosItems.map((id) => {
          ...
          return (
            ...
            >
              <Image
                src={`${DATA_DRAGON_BASE}/img/item/${item.image?.full || 'default.png'}`}
                alt={item.name}
                boxSize="48px"
                borderRadius="md"
              />
  ...
  const renderDecoracion = useCallback(
    (decoracion) => {
      const IconComponent = missionIconMap[decoracion.icon] ?? Sparkles;
      return (
        <Tag
          key={decoracion.key}
          borderRadius="full"
        ...
        {resumenMisiones && (
          <Box borderWidth="1px" borderRadius="2xl" p={6} borderColor="background.muted" bg="background.card" boxShadow="xl">
            ...
            <Stack spacing={4}>
              {misiones.length ? (
                misiones.map((mision) => {
                  const IconComponent = missionIconMap[mision.reward.icon] ?? Sparkles;
                  return (
                    <Box
                      key={mision.key}
                      borderWidth="1px"
          ...
          {favoritosCampeones.length ? (
            <SimpleGrid columns={{ base: 1, sm: 2, md: 3, lg: 4, xl: 5 }} spacing={4}>
              {favoritosCampeones.map((campeon) => {
                const { key, ...propsCampeon } = campeon;
                return (
                  <ChampionCard
                    key={campeon.id}
                    {...propsCampeon}
            ...
            <VStack spacing={3} align="stretch">
              {elementosModal.map((elemento) =>
                renderModalElemento(
                  ...
                  modalTipo === 'items' ? (
                    <Image
                      src={`${DATA_DRAGON_BASE}/img/item/${elemento.image?.full || 'default.png'}`}
                      alt={elemento.name}
                      boxSize="48px"
                      borderRadius="md"
                    />
                    ...
                    <Image
                      src={`https://ddragon.leagueoflegends.com/cdn/img/${elemento.icon}`}
                      alt={elemento.name}
                      boxSize="48px"
                    />
                  )
