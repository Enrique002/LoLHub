...
const Runas = () => {
  const [arbolesRunas, setArbolesRunas] = useState([]);
  const [cargando, setCargando] = useState(true);
  const [terminoBusqueda, setTerminoBusqueda] = useState('');
  const [arbolSeleccionado, setArbolSeleccionado] = useState('todos');
  const [runaSeleccionada, setRunaSeleccionada] = useState(null);
  ...
  useEffect(() => {
    const cargarRunas = async () => {
      try {
        ...
        const arbolesOrdenados = (Array.isArray(respuesta.data) ? respuesta.data : []).sort((a, b) => {
