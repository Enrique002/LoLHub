...
import { missionsService, RankingEntry, MissionReward, RankingResponse } from '../services/missionsService';
...
const Ranking: React.FC = () => {
  ...
  const [resumenUsuario, setResumenUsuario] = useState<RankingResponse['you']>(null);
  ...
  const obtenerDecoracion = (decoracion: MissionReward) => {
    const IconComponent = iconMap[decoracion.icon as keyof typeof iconMap] ?? Sparkles;
    return (
      <Tag
        key={decoracion.key}
        borderRadius="full"
  ...
  const renderAvatarDecorado = (user: RankingEntry['user']) => {
  ...
  const renderEntrada = (entrada: RankingEntry) => {
